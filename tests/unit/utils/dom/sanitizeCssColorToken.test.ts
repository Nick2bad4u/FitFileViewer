...
describe(sanitizeCssColorToken, () => {
    ...
    it("accepts rgb/rgba and hsl/hsla in strict numeric forms", () => {
        ...
        expect(sanitizeCssColorToken("rgba(59,130,246,0.35)", "#000")).toBe(
            "rgba(59,130,246,0.35)"
        );
        expect(sanitizeCssColorToken("hsl(210, 50%, 40%)", "#000")).toBe(
            "hsl(210, 50%, 40%)"
        );
        expect(sanitizeCssColorToken("hsla(210, 50%, 40%, 0.5)", "#000")).toBe(
            "hsla(210, 50%, 40%, 0.5)"
        );
    });
    ...
    it("accepts safe keywords", () => {
        ...
        expect(sanitizeCssColorToken("currentColor", "#000")).toBe(
            "currentColor"
        );
    });
    ...
    it("rejects CSS/attribute injection characters", () => {
        ...
        expect(sanitizeCssColorToken('" onload="alert(1)', "#000")).toBe(
            "#000"
        );
    ...
    it("rejects url()/expression()/@import primitives", () => {
        ...
        expect(sanitizeCssColorToken("@import url(https://evil)", "#000")).toBe(
            "#000"
        );
    });
});
