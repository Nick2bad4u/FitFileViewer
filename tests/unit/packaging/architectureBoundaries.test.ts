...
const directAddFullScreenButtonRuntimeGlobalPattern =
    /\bnew\s+(?:AbortController|MutationObserver)\b|\b(?:document|globalThis|window)\.(?:addEventListener|removeEventListener)\(|\bdocument\.(?:documentElement|exitFullscreen|fullscreenElement|querySelector|readyState)\b|\binstanceof\s+(?:HTMLElement|KeyboardEvent)\b/u;
...
const directMapMeasureToolRuntimeGlobalPattern =
    /\b(?:globalThis|window)\.(?:AbortController|setTimeout|clearTimeout)\b|\bdocument\.(?:addEventListener|createElement|createElementNS|createTextNode|removeEventListener)\b|\binstanceof\s+HTMLElement\b|\bnew\s+AbortController\b|(?:^|[^\w.])(?:setTimeout|clearTimeout)\(/u;
...
const directUiStateManagerBrowserRuntimePattern =
    /\bnew\s+AbortController\b|\bglobalThis\.(?:matchMedia|window)\b|\bwindow\.addEventListener\b|\bdocument\.title\b|\bdocument\.body\.style\.cursor\b/u;
...
describe("architecture boundaries", () => {
    ...
    it("keeps the FIT parser source and facade free of source-level CommonJS exports", () => {
        ...
        const parserRuntimeSource = stripComments(
            readRepositoryFile("electron-app/fitParserRuntime.ts")
        );
        ...
        expect(parserSource).toContain("fitParserRuntime.js");
        expect(parserSource).toContain("type FitParserRuntime");
    ...
    it("keeps migrated main runtime helpers off source-level CommonJS exports", () => {
        ...
        expect(initializeApplicationSource).not.toContain(
            "getAppState as BootstrapMainWindowDependencies"
        );
        ...
        expect(gyazoOAuthServerSource).not.toContain("window.close");
        expect(gyazoOAuthServerSource).not.toContain("<script>");
        expect(setupMainLifecycleSource).toContain(
    ...
    it("keeps preload state leaf modules on native imports", () => {
        ...
        const mainStateApiSource = stripComments(
            readRepositoryFile("electron-app/preload/mainStateApi.ts")
        );
        ...
        expect(mainStateApiSource).toContain("createSafeInvokeHandler");
        expect(mainStateApiSource).not.toContain("ipcRenderer.invoke");
        expect(moduleTypesSource).toContain("MainStateListenRequest");
        expect(moduleTypesSource).toContain("MainStateUnlistenResponse");
    ...
    it("keeps generic storage utilities on provider-based ambient storage lookup", () => {
        ...
        expect(storageUtilsSource).toContain("createDefaultStorageProvider");
        expect(storageUtilsSource).toContain("type StorageUtilsRuntime");
        ...
        expect(storageUtilsRuntimeSource).not.toContain(
            "const localStorageValue = globalThis.localStorage"
        );
        expect(storageUtilsRuntimeSource).not.toContain(
    ...
    it("keeps export utility browser runtime access behind the runtime facade", () => {
        ...
        expect(exportUtilsRuntimeSource).not.toContain("browserGlobal");
        expect(exportUtilsRuntimeSource).not.toContain(
            "confirm?: ConfirmDangerousActionFunction"
        );
        expect(exportUtilsRuntimeSource).not.toContain(
            "localStorage?: ExportStorageLike"
        );
        expect(exportUtilsRuntimeSource).not.toContain(
    ...
    it("keeps renderer runtime globals behind the runtime environment facade", () => {
        ...
        expect(mainUiStartupSource).not.toContain(
            "const mainUiRuntimeEnvironment ="
        );
        expect(mainUiStartupSource).toContain(
    ...
    it("keeps Browser tab entry browser access behind the runtime facade", () => {
        ...
        expect(browserTabSource).not.toMatch(
            /\bdocument\.(?:querySelector|createElement|createTextNode)\b/u
        );
        expect(browserTabSource).not.toMatch(
        ...
        expect(browserTabRuntimeSource).not.toContain("scope.HTMLInputElement");
        expect(browserTabRuntimeSource).not.toContain(
        ...
        expect(browserTabRuntimeSource).toContain(
            "value instanceof getHTMLInputElementConstructor(scope)"
        );
        expect(browserTabRuntimeSource).toContain(
            "value instanceof getHTMLSelectElementConstructor(scope)"
        );
    });
    ...
    it("keeps tab state-manager support on typed state and document access", () => {
        ...
        expect(tabStateManagerDocSource).not.toContain(
            "const tabDocumentRuntime = getTabDocumentRuntime();"
        );
        expect(tabStateManagerSupportSource).toContain(
            "getTabDocumentRuntime().getDocument("
        );
        expect(tabStateManagerDocSource).toContain(
            "getTabDocumentRuntime().getDocument("
        );
        expect(tabStateManagerSupportSource).not.toContain(
            "state/core/stateManager.js"
        ...
        expect(tabStateManagerSupportSource).not.toContain("?? document");
        expect(tabStateManagerDocSource).not.toContain("?? document");
    ...
    it("keeps shared error handling on explicit notification callbacks and typed telemetry", () => {
        ...
        expect(errorHandlingRuntimeSource).not.toContain(
            'Reflect.get(globalThis, "addEventListener")'
        );
        expect(errorHandlingRuntimeSource).not.toContain(
            "readonly AbortController?:"
        );
    ...
    it("keeps accent color picker browser access behind the runtime facade", () => {
        ...
        expect(accentColorPickerSource).toContain("getElements");
        expect(accentColorPickerSource).toContain("isHTMLElement");
        expect(accentColorPickerSource).toContain("isHTMLInputElement");
        expect(accentColorPickerSource).toContain("isHTMLButtonElement");
        expect(accentColorPickerSource).not.toContain(
            "document.addEventListener"
        );
        expect(accentColorPickerSource).not.toContain("document.activeElement");
        expect(accentColorPickerSource).not.toContain(
        ...
        expect(accentColorPickerSource).not.toContain(
            'document.querySelector("#current-theme-name")'
        );
        expect(accentColorPickerSource).not.toContain(
            'document.querySelectorAll(".preset-color")'
        );
        expect(accentColorPickerSource).not.toContain(
    ...
    it("keeps renderer debug logging runtime checks behind the debug runtime adapter", () => {
        ...
        const chartZoomResetPluginSource = stripComments(
            readRepositoryFile(
                "electron-app/utils/charts/plugins/chartZoomResetPlugin.ts"
            )
        );
    ...
    it("keeps animation debug logging clocks behind the runtime facade", () => {
        ...
        expect(lastAnimLogRuntimeSource).not.toContain("globalThis as Partial");
    ...
    it("keeps lifecycle listener cleanup timers and abort controllers behind the runtime adapter", () => {
        ...
        const directLifecycleListenersDownloadGlobalPattern =
            /\bURL\.(?:createObjectURL|revokeObjectURL)\b|\bdocument\.createElement\(\s*["']a["']\s*\)|\bdocument\.body\.(?:append|classList)\b/u;
    ...
    it("keeps migrated chart lifecycle paths on the chart instance registry", () => {
        ...
        expect(renderChartRuntimeHelpersRuntimeSource).not.toContain(
            "getMutableChartRuntimeEnvironment"
        );
        expect(renderChartRuntimeHelpersRuntimeSource).not.toContain(
    ...
    it("keeps Chart.js and DataTables runtime adapters off global symbol registries", () => {
        ...
        const rendererVendorSharedRuntimeSource = stripComments(
            readRepositoryFile(
                "electron-app/renderer/rendererVendorSharedRuntime.ts"
            )
        );
        ...
        expect(rendererVendorSharedSource).toContain(
            "rendererVendorSharedRuntime.js"
        );
        expect(rendererVendorSharedSource).not.toContain(
            "const rendererVendorSharedRuntime ="
        );
        expect(rendererVendorSharedSource).not.toContain("globalThis &");
        expect(rendererVendorSharedSource).not.toContain(
    ...
    it("keeps fullscreen button listener abort-controller creation behind the runtime facade", () => {
        ...
        expect(fullscreenButtonRuntimeSource).toContain(
            "getDocument(scope).createElement"
        );
        expect(fullscreenButtonRuntimeSource).toContain(
    ...
    it("keeps icon SVG element creation behind the icon factory runtime facade", () => {
        ...
        expect(iconFactorySource).toContain("type IconFactoryRuntime");
        expect(iconFactorySource).not.toContain(
    ...
    it("keeps renderer Electron API startup and registration off ambient fallbacks", () => {
        ...
        expect(electronApiStartupHooksSource).not.toContain(
            "extends RendererElectronApiScope"
        );
        expect(electronApiStartupHooksSource).not.toContain(
