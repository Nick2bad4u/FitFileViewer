...
UI state manager system-theme media-query lookup, document title reads/writes, active-file body state updates, active-file display element lookup and span creation, chart-controls element lookup, drop-overlay element lookup, loading indicator/main-content/progress lookup, sidebar element lookup, measurement-mode element lookup, tab button/content lookup, unload-button element lookup, loading cursor updates, listener abort-controller creation, and module-level window
...
`document.body` file-state classes/dataset fields, querying active-file display, chart-controls, drop-overlay, loading-state, sidebar, measurement-mode, tab, or unload-button DOM elements directly, creating active-file display spans directly, checking tab elements against the ambient `HTMLElement` constructor directly, writing
