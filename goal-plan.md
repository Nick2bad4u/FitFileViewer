...
UI state manager system-theme media-query lookup, document title reads/writes, active-file body state updates, loading cursor updates, listener abort-controller creation, and module-level window
...
`document.body` file-state classes/dataset fields, writing `document.body.style.cursor`, registering on
`window.addEventListener`, or constructing `AbortController` directly inside `uiStateManager.ts`, with focused runtime coverage and architecture guardrails
