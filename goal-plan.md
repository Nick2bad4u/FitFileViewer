...
before assertion, so reintroducing `globalApiExposure.ts`, `vendorGlobals.ts`, `mainUiGlobals.ts`, `utils.ts`, or
`utils/legacy` bridge modules fails directly.
...
Export utility Electron API candidate typing now uses explicit readonly method aliases backed by the shared clipboard and
Gyazo API-domain contracts instead of deriving from the monolithic `ElectronAPI` type or carrying a local `Partial`
intersection bridge, with architecture guardrails blocking local `Pick<ElectronAPI, ...>` aliases and `Partial`
candidate bridges from returning.
...
coverage blocking those ad hoc interfaces from returning. Handle-open-file Electron API candidate validation now checks
required method properties through a non-array record guard instead of casting to the local API shape, with focused
malformed candidate coverage and architecture coverage blocking those bridge casts from returning.
...
Its file-opened IPC candidate typing now uses an explicit shared preload-event method alias instead of deriving from
the monolithic `ElectronAPI` type or carrying a local Electron API interface.
...
Lifecycle listener export tests and UI state renderability tests now describe active FIT raw data instead of retired
`globalData` state, with architecture coverage blocking that stale focused vocabulary from returning.
...
probes, and lifecycle Electron API candidate validation now checks optional method properties through a non-array
record guard instead of casting to the local API shape, with focused malformed candidate coverage and architecture
coverage blocking that cast from returning.
...
guard over an unknown-valued candidate instead of casting the renderer Electron API candidate to a generic record, with
strict coverage for malformed `installUpdate` values and architecture coverage blocking that bridge cast from returning.
Update notification restart-action candidate typing now uses an exact readonly method alias backed by the shared
menu-event API-domain contract instead of deriving from the monolithic `ElectronAPI` type or carrying a local Electron
API interface.
...
Nested process/env property reads now use guarded typed indexed access on a named runtime property candidate instead of
a local `Reflect.get` probe or generic `Record<string, unknown>` cast.
...
`RendererElectronApiScope` arguments, ambient global fallbacks, or array-shaped compatibility records, with runtime
coverage and architecture coverage blocking those compatibility shapes from returning.
...
Chart state-view selected-chart and controls-visible reads now route through renderer chart state-domain helpers instead
...
The chart runtime dependency resolver no longer exposes generic `gs_rcwd`, `ss_rcwd`, or `us_rcwd` state aliases; chart
render code must use typed state-domain helpers or direct explicit state API dependencies instead of carrying generic
state-manager methods through render dependencies.
...
Inline zone selector delayed-update bookkeeping now stores handles as the selector runtime's
`CreateInlineZoneColorSelectorTimerHandle` alias instead of spelling `ReturnType<typeof setTimeout>` in the source
module, with architecture guardrails blocking that ambient timer-handle type spelling from returning.
