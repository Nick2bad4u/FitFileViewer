...
Use Vitest for app tests. Prefer `*.test.ts` for new Vitest tests. Put app-coupled tests under the relevant `electron-app/tests/` area, but keep root-owned suites such as packaging and shared tab behavior under `tests/unit/`. Place Electron Playwright smoke tests under `tests/playwright/`. Use `tests/fixtures/` for reusable Vitest fixtures and `fit-test-files/` for real FIT-file coverage. Run targeted tests while developing, then run `npm test` before opening a PR.
