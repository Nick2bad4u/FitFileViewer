...
- Current status: shrinking; renderer bootstrap now has focused modules for file input startup, development debug globals, Electron API registration, error handling, and lifecycle cleanup; sync renderer notifications and loading now live in focused UI modules that `rendererUtils` re-exports for compatibility, state domain modules are guarded against importing broad renderer utilities, and `renderer.ts` now uses the focused loading helper directly
- Next removal step: replace remaining direct `rendererUtils` imports from state/core initialization code with narrower renderer initialization services
- Verification gates:
  - `npm run lint:app`
  - `npm test -- tests/unit/packaging/architectureBoundaries.test.ts`
