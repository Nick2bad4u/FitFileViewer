...
- Current status: required by current Electron packaging and preload bundling path; domain-specific preload files exist but still export CommonJS-compatible surfaces, dev-only global exposure now lives in `electron-app/preload/developmentToolsGlobal.ts` instead of the root preload composition file, preload module loading now lives in `electron-app/preload/preloadModuleLoader.ts` so the root preload entrypoint no longer owns every helper `require(...)` call, exposed `electronAPI` method assembly now lives in `electron-app/preload/electronApiFactory.ts`, and shared preload module registry typing now lives in `electron-app/preload/preloadModuleTypes.d.ts` without adding a runtime dependency
