 ...
 * @property {Function} beforeDraw - Draws the background color before chart
 *   rendering
 */
/**
 * Lightweight structural JSDoc to avoid heavy chart.js type imports (which were
 * causing TS2307 in checkJs mode). We define the minimal shape we actually rely
 * on instead of importing from the chart.js module.
 *
...
export const chartBackgroundColorPlugin = {
    ...
    beforeDraw: (chart, options) => {
        ...
        if (!backgroundColor) {
            try {
                ...
                const pluginCfg =
                    ...
                    chart.options.plugins.chartBackgroundColorPlugin;
                if (
                    pluginCfg &&
                    typeof pluginCfg.backgroundColor === "string"
                ) {
        ...
        if (!backgroundColor) {
            console.warn(
                "[chartBackgroundColorPlugin] No backgroundColor set, using default #23263a"
            );
            backgroundColor = "#23263a";
        }
        const ctx = chart?.ctx,
        ...
        const w = /** @type {any} */ (
            globalThis.window === undefined ? undefined : globalThis
        );
