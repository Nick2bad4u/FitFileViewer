...
export function renderZoneChart(
    ...
    try {
        ...
        /** @type {string[]} */
        let colors = Array.isArray(themeConfig?.colors?.zoneColors)
            ? themeConfig.colors.zoneColors
            : [];
        ...
        if (zoneData.length > 0 && typeof zoneData[0]?.color === "string") {
            ...
            if (zoneType) {
                colors =
                    getChartZoneColors(zoneType, zoneData.length) || colors;
            }
        }
...
function createBarChartConfig(
    ...
    return {
        data: {
            ...
            labels: zoneData.map(
                (zone, i) => zone.label || `Zone ${zone.zone || i + 1}`
            ),
        },
        options: {
            animation: {
...
function createChartConfig(
    ...
    const baseDataset = {
        ...
        data: zoneData.map((zone) =>
            typeof zone.time === "number" ? zone.time : 0
        ),
    };
