...
export function buildSummaryText(result, config, stats) {
    ...
    if (result.mode === "valueRange") {
        ...
        return `Showing ${result.selectedCount} of ${result.totalCandidates} points between ${formatMetricValue(appliedMin, stats ?? null)} and ${formatMetricValue(appliedMax, stats ?? null)} ${result.metricLabel ?? result.metric} (${coverage}% coverage)`;
    }
    const percentValue = config?.percent ?? result.percent ?? 0;
    return `Showing top ${percentValue}% (${result.selectedCount} of ${result.totalCandidates}) by ${result.metricLabel ?? result.metric}`;
}
