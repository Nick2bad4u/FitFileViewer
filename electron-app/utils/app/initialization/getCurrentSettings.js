...
function isResettable(el) {
    ...
    return typeof el._updateFromReset === "function";
}
...
const resetTimerHandles = new Set();
/**
...
export function getCurrentSettings() {
    try {
        ...
        const fields = Array.isArray(formatChartFields)
            ? formatChartFields.filter((field) => typeof field === "string")
            : [];
        const colorDefaults = fieldColors;
...
function clearScheduledResetTimers() {
    for (const handle of resetTimerHandles) {
        clearTimeout(handle);
    }
    resetTimerHandles.clear();
}
function scheduleResetTimer(callback, delay) {
...
function resetUIControlsToDefaults(wrapper) {
    ...
    try {
        ...
        const defaultFieldColors = fieldColors;
