...
export function drawOverlayForFitFile({ endIcon, fileName, fitData, formatTooltipData, getLapNumForIdx, map, markerClusterGroup, overlayIdx, startIcon, }) {
    ...
    const coords = recordMesgs
        .map((row, idx) => {
        if (typeof row.positionLat === "number" &&
    ...
    if (coords.length > 0) {
        ...
        const paletteColor = Array.isArray(chartOverlayColorPalette) &&
            ...
            : "#1976d2";
        const polyline = L.polyline(coords.map((c) => [c[0], c[1]]), buildPolylineOptions({
        ...
        if (!resultBounds) {
            try {
                const lb = L && typeof L.latLngBounds === "function"
                    ? L.latLngBounds(pts)
                    : null;
                resultBounds =
 ...
 */
export function mapDrawLaps(lapIdx, { baseLayers: _baseLayers, endIcon, formatTooltipData, getLapColor, getLapNumForIdx, map, mapContainer, markerClusterGroup, startIcon, }) {
    ...
    console.log("[mapDrawLaps] ENTERED FUNCTION, lapIdx =", lapIdx, "type:", typeof lapIdx, Array.isArray(lapIdx) ? "isArray" : "notArray");
    let coords = [];
    ...
    const lapMesgs = (getWin().globalData?.lapMesgs || []), recordMesgs = (getWin().globalData?.recordMesgs || []);
    patchLapIndices(lapMesgs, recordMesgs);
    ...
    if (lapIdx === "all" || (Array.isArray(lapIdx) && lapIdx.includes("all"))) {
        ...
        coords = recordMesgs
            ...
            if (row &&
                ...
                if (idx < 10 || idx > recordMesgs.length - 10) {
                    console.log(`[mapDrawLaps] idx=${idx}, lapNum=${lapNum}, lat=${row.positionLat}, lon=${row.positionLong}`);
                }
                return [
                    Number((row.positionLat / 2 ** 31) * 180),
                    Number((row.positionLong / 2 ** 31) * 180),
        ...
        if (getLoadedFitFiles().length > 1) {
            ...
            for (let i = 1; i < loaded.length; ++i) {
                ...
                const color = colorPalette[overlayIdx % colorPalette.length], fileName = typeof getOverlayFileName === "function"
                    ? getOverlayFileName(i)
                    : overlay.filePath || "", bounds = drawOverlayForFitFile({
                    color: color || "#1976d2",
                    endIcon,
                    fileName,
                    fitData: {
        ...
        for (const lapVal of lapIdx) {
            ...
            if (lap &&
                ...
                if (startIdx !== -1 && endIdx !== -1 && startIdx <= endIdx) {
                    const lapRecords = recordMesgs.slice(startIdx, endIdx + 1), lapCoords = lapRecords
                        .map((row, idx) => {
                        if (typeof row.positionLat === "number" &&
                            typeof row.positionLong === "number") {
                            return [
                    ...
                    if (lapCoords.length > 0) {
                        const polyColor = getLapColor(Number(lapVal)), polyline = L.polyline(lapCoords.map((c) => [c[0], c[1]]), buildPolylineOptions({
                            color: polyColor,
                            dashArray: null,
                            opacity: 0.9,
                            weight: 4,
    ...
    if (coords.length > 0) {
        ...
        const polylineResult = L.polyline(coords.map((c) => [c[0], c[1]]), buildPolylineOptions({
            color: polyColor,
...
function findClosestRecordIndexByLatLon(lat, lon, records) {
    ...
    for (const [i, r] of records.entries()) {
        if (r &&
            ...
            const dLat = r.positionLat - lat, dLon = r.positionLong - lon, dist = dLat * dLat + dLon * dLon;
            if (dist < minDist) {
                minDist = dist;
                minIdx = i;
            }
...
function selectMarkerCoordinatesForDataset(coordsArray, shouldUpdateSummary = true) {
    ...
    const { appliedMax, appliedMin, maxCandidate, metric, metricLabel, minCandidate, mode, orderedIndices, percent, threshold, } = filterResult;
