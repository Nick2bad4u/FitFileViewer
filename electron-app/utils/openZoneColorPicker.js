...
export function openZoneColorPicker(field) {
	try {
		...
		zoneData.forEach((zone, index) => {
			...
			const currentColor = localStorage.getItem(`chartjs_${zoneType.toLowerCase()}_zone_${index + 1}_color`) ||
				defaultColors[index] || defaultColors[index % defaultColors.length];
