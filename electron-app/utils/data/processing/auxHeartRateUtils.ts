...
/**
 * Resolution metadata for an auxiliary heart rate source.
 */
export interface AuxHeartRateResolution {
    developerFieldIds?: string[];
    fieldKey?: string;
    fieldName?: string;
    source: "developerField" | "none" | "recordKey";
}

/**
 * Result from applying auxiliary heart rate normalization to record messages.
 */
export interface ApplyAuxHeartRateResult {
    applied: boolean;
    resolution: AuxHeartRateResolution;
    valuesFound: number;
}

interface CachedAuxHeartRateResolution {
    fieldDescriptionMesgs: readonly unknown[];
    resolution: AuxHeartRateResolution;
}

interface DeveloperFieldMatch {
    fieldIds: string[];
    fieldName: string;
}

interface NormalizedFieldDescription {
    developerDataIndex: string | null;
    fieldDefinitionNumber: string | null;
    fieldName: string | null;
}

interface RecordKeyScanResult {
    count: number;
    key: string | null;
}
...
export function resolveFieldDescriptionMessages(
    fitData: DataRecord | null | undefined
): unknown[] {
...
function isAuxHeartRateKeyCandidate(key: string): boolean {
    if (!key) {
        return false;
    }
...
function countNumericValues(recordMesgs: readonly DataRecord[], key: string) {
    let count = 0;
...
function scanRecordKeysForAuxHeartRate(
    recordMesgs: readonly DataRecord[]
): RecordKeyScanResult {
...
function resolveAuxDeveloperField(
    fieldDescriptionMesgs: readonly unknown[]
): DeveloperFieldMatch | null {
...
function getDeveloperFieldValue(
    row: DataRecord,
    ...
    let devFields: DataRecord | null = null;

    if (typeof devFieldValue === "string") {
        try {
            const parsed: unknown = JSON.parse(devFieldValue);
...
export function resolveAuxHeartRateResolution(
    ...
    if (candidateKey && count > 0) {
        const resolution: AuxHeartRateResolution = {
    ...
    const resolution: AuxHeartRateResolution = { source: "none" };
    AUX_HEART_RATE_CACHE.set(recordMesgs, {
        fieldDescriptionMesgs,
        resolution,
    });
...
function countDeveloperFieldValues(
    recordMesgs: readonly DataRecord[],
    fieldIds: readonly string[]
) {
    let count = 0;
