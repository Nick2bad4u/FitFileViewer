/**
 * @fileoverview Unified State Management Facade
 * @description Provides a consistent interface across all state management systems during migration
 ...
 * @property {(path: string) => boolean} isLegacyPath - Check if path belongs to legacy system
 * @property {(path: string) => any} getLegacyState - Get state from legacy system
 * @property {(path: string, value: any) => void} setLegacyState - Set state in legacy system
 */
 ...
 * @property {boolean} [silent=false] - Whether to suppress change notifications
 */
...
export class UnifiedStateManager {
    ...
    getLegacyState(path, defaultValue) {
        ...
        if (globalThis.window !== undefined && globalThis.window.globalData && path === "globalData") {
        ...
        try {
            if (this.isLegacyPath(path)) {
                ...
                if (opts.syncLegacy && this.syncEnabled) {
                    setNewState(path, value, { source: `${opts.source}-legacy-sync`, silent: opts.silent });
                }
            } else {
                setNewState(path, value, { source: opts.source, silent: opts.silent });
            }
    ...
    setLegacyState(path, value, _options) {
        // Show deprecation warning once per path
        if (!this.legacyWarningsShown.has(path)) {
            console.warn(`[UnifiedState] Setting legacy state path "${path}". Consider migrating to new state system.`);
            this.legacyWarningsShown.add(path);
        }
    ...
    validateConsistency() {
        ...
        for (const legacyPath of LEGACY_PATHS) {
            try {
                ...
                if (legacyValue !== undefined && newValue !== undefined && legacyValue !== newValue) {
                    issues.push({
                        legacyValue,
                        newValue,
                        path: legacyPath,
