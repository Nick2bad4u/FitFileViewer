...
describe('ChartStateManager', () => {
    ...
    describe('Debounced Rendering', () => {
        it('should debounce multiple render calls', async () => {
            const performRenderSpy = vi.spyOn(chartStateManager, 'performChartRender').mockImplementation(() => Promise.resolve());
    ...
    describe('Tab Activation Handling', () => {
        it('should handle tab activation', () => {
            ...
            expect(setState).toHaveBeenCalledWith('charts.tabActive', true, { source: 'ChartStateManager.handleTabActivation' });
    ...
    describe('Chart State Management', () => {
        it('should clear chart state', () => {
            const destroyExistingChartsSpy = vi.spyOn(chartStateManager, 'destroyExistingCharts').mockImplementation(() => {});
        ...
        it('should handle chart destruction errors', () => {
            ...
            const mockCharts = [
                { destroy: vi.fn() },
                { destroy: vi.fn().mockImplementation(() => { throw new Error('Destroy failed'); }) },
                { destroy: vi.fn() }
            ];
            (globalThis as any)._chartjsInstances = mockCharts;
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith('[ChartStateManager] Error destroying chart 1:', expect.any(Error));
            expect((globalThis as any)._chartjsInstances).toEqual([]);
    ...
    describe('Chart Rendering', () => {
        it('should perform chart render successfully', async () => {
            ...
            expect(setState).toHaveBeenCalledWith('charts.isRendering', true, { source: 'ChartStateManager.performChartRender' });
            expect(destroyExistingChartsSpy).toHaveBeenCalled();
            expect(renderChartJS).toHaveBeenCalled();
            expect(setState).toHaveBeenCalledWith('charts.lastRenderTime', expect.any(Number), { source: 'ChartStateManager.performChartRender' });
            expect(setState).toHaveBeenCalledWith('charts.isRendering', false, { source: 'ChartStateManager.performChartRender' });
        ...
        it('should handle chart render errors', async () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith('[ChartStateManager] Error during chart rendering:', expect.any(Error));
