...
function injectRenderChartJSMocks() {
    ...
    const mocks = {
        ...
        // Theme management
        theme: { getThemeConfig: vi.fn().mockReturnValue({
            colors: {
                text: '#000000',
                textPrimary: '#333333',
        ...
        createEnhancedChart: { createEnhancedChart: vi.fn().mockReturnValue({
            destroy: vi.fn(),
            update: vi.fn(),
...
describe('renderChartJS.js - Comprehensive Coverage with Module Cache Injection', () => {
    ...
    describe('chartSettingsManager Object', () => {
        ...
        it('should provide setFieldVisibility with state updates', async () => {
            ...
            expect(mocks.computedStateManager.computedStateManager.invalidateComputed).toHaveBeenCalledWith('charts.renderableFieldCount');
        });
    ...
    describe('chartActions Object - State Management', () => {
        ...
        it('should select chart and trigger re-render when rendered', async () => {
            ...
            expect(mocks.stateManager.setState).toHaveBeenCalledWith('charts.selectedChart', 'power', expect.any(Object));
        });
        ...
        it('should toggle controls visibility', async () => {
            ...
            expect(mocks.stateManager.setState).toHaveBeenCalledWith('charts.controlsVisible', false, expect.any(Object));
            expect(mocks.uiStateManager.uiStateManager.updatePanelVisibility).toHaveBeenCalledWith('chart-controls', false);
        });
    });
    ...
    describe('Main renderChartJS Function - Core Rendering', () => {
        ...
        it('should handle no valid data scenario', async () => {
            ...
            expect(mocks.showNotification.showNotification).toHaveBeenCalledWith('No FIT file data available for chart rendering', 'warning');
            expect(result).toBe(false);
        });
        ...
        it('should handle Chart.js not available error', async () => {
            ...
            expect(mocks.showNotification.showNotification).toHaveBeenCalledWith('Chart library not available', 'error');
            expect(result).toBe(false);
        });
        ...
        it('should handle empty record messages', async () => {
            ...
            expect(mocks.showNotification.showNotification).toHaveBeenCalledWith('No chartable data found in this FIT file', 'info');
            expect(result).toBe(false);
        });
        ...
        it('should handle critical error in chart rendering', async () => {
            ...
            expect(mocks.showNotification.showNotification).toHaveBeenCalledWith('Failed to render charts due to an error', 'error');
            expect(result).toBe(false);
        });
