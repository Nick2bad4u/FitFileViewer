...
vi.mock('../../../utils/state/core/stateManager.js', () => ({
    getState: vi.fn((path) => {
        ...
        let current = Object.fromEntries(mockState);
    ...
    setState: vi.fn((path, value) => {
        mockState.set(path, value);
        // Trigger any subscriptions for this path
        const subscription = mockSubscriptions.get(path);
        if (subscription) {
            subscription(value);
        }
    }),
    updateState: vi.fn((path, value) => {
        const currentValue = mockState.get(path) || {};
        const newValue = { ...currentValue, ...value };
        mockState.set(path, newValue);
    }),
    subscribe: vi.fn((path, callback) => {
        mockSubscriptions.set(path, callback);
        return () => mockSubscriptions.delete(path);
    })
}));
...
describe('renderChartJS.js - Comprehensive Utility Function Coverage', () => {
    ...
    describe('refreshChartsIfNeeded function - Conditional Refresh Logic', () => {
        test('should return true and trigger refresh when conditions are met', () => {
            ...
            mockState.set('globalData', {
                recordMesgs: [1, 2, 3] // Has data
            });
            mockState.set('charts.isRendering', false);
        ...
        test('should return false when currently rendering', () => {
            ...
            mockState.set('charts.isRendering', true);
    ...
    describe('getChartStatus function - Status Information Retrieval', () => {
        test('should return comprehensive chart status object', () => {
            // Set up mock state
            mockState.set('charts.isRendered', true);
            mockState.set('charts.isRendering', false);
            mockState.set('charts.controlsVisible', true);
            mockState.set('charts.selectedChart', 'power');
            mockState.set('charts.renderedCount', 8);
            mockState.set('charts.lastRenderTime', 1234567890);
            mockState.set('performance.renderTimes.chart', 150);
            mockState.set('globalData', {
                recordMesgs: [1, 2, 3]
            });
        ...
        test('should correctly detect hasData with various data states', () => {
            ...
            mockState.set('globalData', undefined);
            expect((getChartStatus() as any).hasData).toBe(false);

            // Test with empty recordMesgs
            mockState.set('globalData', { recordMesgs: [] });
            expect((getChartStatus() as any).hasData).toBe(false);

            // Test with valid recordMesgs
            mockState.set('globalData', { recordMesgs: [1, 2, 3] });
    ...
    describe('chartState object - State Getters', () => {
        test('should correctly get isRendered state', () => {
            ...
            mockState.set('charts.isRendered', false);
            expect(chartState.isRendered).toBe(false);

            mockState.set('charts.isRendered', undefined);
            expect(chartState.isRendered).toBe(false);
        });
        ...
        test('should correctly get isRendering state', () => {
            ...
            mockState.set('charts.isRendering', undefined);
            expect(chartState.isRendering).toBe(false);
        });
        ...
        test('should correctly get controlsVisible with default true', () => {
            ...
            mockState.set('charts.controlsVisible', false);
            expect(chartState.controlsVisible).toBe(false);

            // Test default behavior when undefined
            mockState.set('charts.controlsVisible', undefined);
            expect(chartState.controlsVisible).toBe(true);
        });
        ...
        test('should correctly get selectedChart with default', () => {
            ...
            mockState.set('charts.selectedChart', 'heart_rate');
            expect(chartState.selectedChart).toBe('heart_rate');

            // Test default when undefined
            mockState.set('charts.selectedChart', undefined);
        ...
        test('should correctly detect hasValidData', () => {
            ...
            mockState.set('globalData', { recordMesgs: null });
            expect(chartState.hasValidData).toBe(false);
