...
describe("summaryColModal - file override persistence", () => {
    beforeEach(() => {
        ...
        (globalThis.window as any).globalData = { cachedFilePath: "C:/tmp/activity.fit" };
    });
    ...
    it("clears per-file override when reset to default", async () => {
        const { getStorageKey, getGlobalStorageKey, saveColPrefs, loadColPrefs, orderSummaryColumnsNamedFirst } =
            await import(HELPERS);
        const { showColModal } = await import(MODAL);

        const allKeys = ["Speed", "Distance", "HeartRate"];

        const fileKey = getStorageKey((globalThis.window as any).globalData, allKeys);
        const globalKey = getGlobalStorageKey();

        // Global default differs from file override.
        const globalDefault = orderSummaryColumnsNamedFirst(allKeys).filter((k) => ["Speed", "Distance"].includes(k));
        const fileOverride = orderSummaryColumnsNamedFirst(allKeys).filter((k) => ["Speed"].includes(k));
        ...
        const resetBtn = Array.from(overlay.querySelectorAll<HTMLButtonElement>("button")).find(
