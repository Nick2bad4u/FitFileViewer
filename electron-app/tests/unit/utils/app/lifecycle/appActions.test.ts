...
describe('AppActions', () => {
  ...
  itHasAssertions('loadFile should set loading, update state, performance, notify, and always clear loading', async () => {
  ...
  itHasAssertions('renderChart should update charts slice and performance', () => {
    ...
    expect(h.mockUpdateState).toHaveBeenCalledWith(
      'charts',
      expect.objectContaining({ isRendered: true, chartData: { datasets: [] }, chartOptions: { responsive: true } }),
      expect.any(Object)
    );
    expect(h.mockUpdateState).toHaveBeenCalledWith(
