...
describe("chartZoneColorUtils", () => {
    ...
    it("getZoneColor falls back to defaults and wraps index", async () => {
        ...
        expect(getZoneColor("hr", 7)).toBe(
            DEFAULT_HR_ZONE_COLORS[7 % DEFAULT_HR_ZONE_COLORS.length]
        );

        expect(getZoneColor("power", 0)).toBe(DEFAULT_POWER_ZONE_COLORS[0]);
        expect(getZoneColor("power", 10)).toBe(
            DEFAULT_POWER_ZONE_COLORS[10 % DEFAULT_POWER_ZONE_COLORS.length]
        );
    });
    ...
    it("getChartSpecificZoneColor prefers chart-specific then zone default", async () => {
        ...
        expect(getChartSpecificZoneColor("hr_zone_doughnut", 0)).toBe(
            "#111111"
        );
    ...
    it("resetChartSpecificZoneColors sets scheme to custom and writes defaults", async () => {
        ...
        // Scheme flag
        expect(
            localStorage.getItem("chartjs_hr_zone_doughnut_color_scheme")
        ).toBe("custom");
        // Default colors written
        expect(
            localStorage.getItem("chartjs_hr_zone_doughnut_zone_1_color")
        ).toBe("#111111");
        expect(
            localStorage.getItem("chartjs_hr_zone_doughnut_zone_2_color")
        ).toBe("#222222");
        expect(
            localStorage.getItem("chartjs_hr_zone_doughnut_zone_3_color")
        ).toBe("#333333");
    });
    ...
    it("getChartZoneColors returns scheme colors for named schemes and custom uses saved/defaults", async () => {
        ...
        expect(vibrantPower).toEqual(
            (schemes.vibrant.power as string[]).slice(0, 4)
        );
