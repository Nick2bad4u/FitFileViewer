...
const { mockGetState, mockSetState, mockSubscribe } = /** @type {any} */ (
  vi.hoisted(() => ({
    mockGetState: vi.fn(),
    mockSetState: vi.fn(),
    mockSubscribe: vi.fn(),
...
describe("tabStateManager.behavior", () => {
  ...
  it("handleChartTab sets charts.tabActive true regardless of render state", async () => {
    ...
    mockGetState.mockImplementation((/** @type {any} */ key) => (key === "charts" ? { isRendered: true } : { recordMesgs: [{}] }));
    await tabStateManager.handleChartTab({ recordMesgs: [{}] });
    expect(mockSetState).toHaveBeenCalledWith(
      "charts.tabActive",
      true,
  ...
  /** @type {any} */ (window).renderMap = vi.fn();
    mockGetState.mockImplementation((/** @type {any} */ key) => (key === "map" ? { isRendered: true } : { recordMesgs: [{}] }));
    await tabStateManager.handleMapTab({ recordMesgs: [{}] });
  ...
  it("handleTabSpecificLogic executes 'data' branch and calls createTables when no background content", async () => {
    ...
    mockGetState.mockImplementation((/** @type {any} */ key) => (key === "globalData" ? { recordMesgs: [{}] } : null));
