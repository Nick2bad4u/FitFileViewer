...
describe('main.js - Targeted 100% Coverage Test', () => {
    test('should achieve 100% coverage through systematic function targeting', async () => {
        ...
        globalMocks.mockFs.writeFileSync.mockImplementationOnce(() => { throw new Error('Permission denied'); });
        ...
        globalMocks.mockHttps.request.mockImplementationOnce(() => { throw new Error('Network error'); });

        // Dialog errors
        globalMocks.mockDialog.showOpenDialog.mockImplementationOnce(() => Promise.reject(new Error('Dialog error')));
        globalMocks.mockDialog.showSaveDialog.mockImplementationOnce(() => Promise.reject(new Error('Save error')));

        // Window creation errors
        globalMocks.MockBrowserWindow.mockImplementationOnce(() => { throw new Error('Window creation failed'); });
