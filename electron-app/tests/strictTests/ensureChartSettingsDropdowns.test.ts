...
vi.mock('../../utils/state/core/stateManager.js', () => ({
  ...
  updateState: (ns: string, patch: any) => { state[ns] = { ...(state[ns]||{}), ...patch }; },
}));
...
function seedGlobalData() {
  (window as any).globalData = {
    recordMesgs: [
      { speed: 1.2, heartRate: 120, altitude: 10, power: 150, cadence: 90, temperature: 22, distance: 100, enhancedSpeed: 1.3, enhancedAltitude: 11, resistance: 1, flow: 2, grit: 3, positionLat: 1, positionLong: 2, dev_field1: 5 },
      { speed: 2.2, heartRate: 130, altitude: 12, power: 170, cadence: 88, temperature: 24, distance: 150, enhancedSpeed: 1.8, enhancedAltitude: 12, resistance: 2, flow: 3, grit: 4, positionLat: 1.1, positionLong: 2.2, dev_field1: 7 },
    ],
    timeInZoneMesgs: [
...
describe('ensureChartSettingsDropdowns integration', () => {
  ...
  it('toggle all buttons update all fields and notify', () => {
    ...
    const wrapper = document.getElementById('chartjs-settings-wrapper')!;
    const enableAll = Array.from(wrapper.querySelectorAll('button')).find(b => b.textContent?.includes('Enable All')) as HTMLButtonElement;
    const disableAll = Array.from(wrapper.querySelectorAll('button')).find(b => b.textContent?.includes('Disable All')) as HTMLButtonElement;
  ...
  it('export section: no charts warns; with charts opens modal and calls export utils', () => {
    ...
    // No charts path
    delete (window as any)._chartjsInstances;
    const exportZipBtn = Array.from(wrapper.querySelectorAll('button')).find(b => b.textContent?.includes('Export ZIP')) as HTMLButtonElement;
    ...
    const savePngBtn = Array.from(wrapper.querySelectorAll('button')).find(b => b.textContent?.includes('Save PNG')) as HTMLButtonElement;
  ...
  const chartButtons = Array.from(overlay!.querySelectorAll('button')).filter(b => b.textContent?.startsWith('📊'));
    ...
    savePngBtn.click();
    const combinedBtn = Array.from(document.querySelectorAll('button')).find(b => b.textContent?.startsWith('🔗 All Charts Combined')) as HTMLButtonElement;
  ...
  it('settings header reset button calls reset and re-enables after delay', () => {
    ...
    const resetBtn = Array.from(header.querySelectorAll('button')).find(b => b.textContent?.includes('Reset')) as HTMLButtonElement;
