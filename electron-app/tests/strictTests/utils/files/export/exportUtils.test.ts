...
describe("exportUtils core flows", () => {
  ...
  it("exportCombinedChartsDataAsCSV merges timestamps across charts", async () => {
    ...
    const chartA: any = {
      data: { datasets: [{ label: "A", data: [{ x: 1, y: 10 }, { x: 3, y: 30 }] }] },
      canvas: { width: 100, height: 50 },
    };
    const chartB: any = {
      data: { datasets: [{ label: "B", data: [{ x: 2, y: 20 }, { x: 3, y: 33 }] }] },
      canvas: { width: 100, height: 50 },
    };
  ...
  it("uploadToImgur throws when client id is not configured", async () => {
    const { exportUtils } = await import(modPath);
    await expect(exportUtils.uploadToImgur("data:image/png;base64,AAA")).rejects.toThrow(/Imgur client ID not configured/i);
  });
  ...
  it("addCombinedCSVToZip creates combined-data.csv with union timestamps", async () => {
    ...
    const chartA: any = { data: { datasets: [{ label: "A", data: [{ x: 1, y: 10 }, { x: 3, y: 30 }] }] } };
    const chartB: any = { data: { datasets: [{ label: "B", data: [{ x: 2, y: 20 }, { x: 3, y: 33 }] }] } };
