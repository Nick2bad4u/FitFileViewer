...
vi.mock("../../charts/core/chartNotificationState.js", () => ({
    ...
    updatePreviousChartState: vi.fn(
        (chartCount: number, fieldsCount: number, now: number) => {
            sharedState.chartCount = chartCount;
            sharedState.fieldsRendered = new Array(fieldsCount).fill(
                true
            ) as any[];
...
describe("showRenderNotification strict", () => {
    ...
    it("shows for significant visible field change (>2)", async () => {
        const { updatePreviousChartState } =
            await import("../../utils/charts/core/chartNotificationState.js");
