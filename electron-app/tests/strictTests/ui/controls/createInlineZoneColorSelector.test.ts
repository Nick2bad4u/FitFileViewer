...
const hoisted = vi.hoisted(() => {
  ...
  const getChartSpecificZoneColor = vi.fn((field: string, idx: number) => storeChartSpecific[`${field}:${idx}`] || "#000000");
  const saveChartSpecificZoneColor = vi.fn((field: string, idx: number, val: string) => { storeChartSpecific[`${field}:${idx}`] = val; });
  const saveZoneColor = vi.fn((type: string, idx: number, val: string) => { storeGeneric[`${type}:${idx}`] = val; });
  const resetChartSpecificZoneColors = vi.fn((field: string, count: number) => { for (let i = 0; i < count; i++) delete storeChartSpecific[`${field}:${i}`]; });
  const resetZoneColors = vi.fn((type: string, count: number) => { for (let i = 0; i < count; i++) delete storeGeneric[`${type}:${i}`]; });
  const getChartZoneColors = vi.fn((zoneType: string, count: number, scheme: string) => Array.from({ length: count }, (_, i) => `#S-${scheme}-${i}`));
...
// Under test
import { createInlineZoneColorSelector, removeInlineZoneColorSelectors, updateInlineZoneColorSelectors, clearZoneColorData, getCurrentColorScheme } from "../../../../utils/ui/controls/createInlineZoneColorSelector.js";
