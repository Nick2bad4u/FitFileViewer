...
import type { SummaryRenderData } from "../../../../utils/rendering/core/renderSummary.js";
import type { FitSummaryData } from "../../../../utils/rendering/helpers/renderSummaryHelpers.js";
...
describe("renderSummary helpers + renderSummary", () => {
    ...
    it("getStorageKey prefers window.globalData.cachedFilePath then data.cachedFilePath then activeFitFileName", async () => {
        ...
        expect(
            getStorageKey({ cachedFilePath: "/a/b/c.fit" }, [])
        ).toBe(`summaryColSel_${encodeURIComponent("/a/b/c.fit")}`);

        summaryWindow.globalData = { cachedFilePath: "C:/tmp/foo.fit" };
        expect(
            getStorageKey({ cachedFilePath: "/a/b/c.fit" }, [])
        ).toBe(`summaryColSel_${encodeURIComponent("C:/tmp/foo.fit")}`);
