...
vi.mock("../../../../utils/maps/layers/mapDrawLaps.js", () => ({ mapDrawLaps: vi.fn(), drawOverlayForFitFile: vi.fn() }));
...
vi.mock("../../../../utils/ui/controls/createElevationProfileButton.js", () => ({ createElevationProfileButton: vi.fn() }));
...
describe("renderMap core", () => {
  ...
  it("creates map structure and UI controls, sets up zoom slider and layers button", async () => {
    ...
    expect(Number(slider.value)).toBe(Math.round((map.getZoom() - map.getMinZoom()) / (map.getMaxZoom() - map.getMinZoom()) * 100));
    ...
    expect(slider.value).toBe(String(Math.round((map.getZoom() - map.getMinZoom()) / (map.getMaxZoom() - map.getMinZoom()) * 100)));
  });
