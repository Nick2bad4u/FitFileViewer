...
async function initializeAsyncComponents() {
    try {
        // Initialize quick color switcher (theme and accent color selector)
        try {
            const { initQuickColorSwitcher } = await import("./utils/ui/quickColorSwitcher.js");
            initQuickColorSwitcher();
            console.log("[Renderer] Quick color switcher initialized");
        } catch (error) {
            console.warn("[Renderer] Quick color switcher initialization failed:", error);
        }
...
if (isDevelopmentMode()) {
    ...
    (async () => {
        try {
            ...
            console.log("[Charts] Sensor Debug Commands:");
