...
function logWithContext(level, message, context = {}) {
    ...
    const getConsoleMethod = (lvl) => {
        ...
        return typeof m === "function" ? m.bind(console) : console.log.bind(console);
    };

    /**
     * Decide whether a key should be redacted.
     *
     * NOTE: We intentionally do not redact *all* keys containing "token" because
     * configuration keys like tokenUrl are not secrets. We do redact actual token
     * values and secrets.
     *
     * @param {string} key
     * @returns {boolean}
    ...
    const shouldRedactKey = (key) => {
        ...
        if (k === "authorization" || k === "cookie" || k === "set-cookie" || k.includes("password")) return true;
