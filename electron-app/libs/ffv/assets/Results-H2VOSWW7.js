		...
		children: [
			...
			c.jsx('div', {
				children: t.fileHeaders.map(
					({ index: s, dataSize: i, protocolVersion: a, profileVersion: u }) =>
						c.jsxs(
							'dl',
							{
...
function Pn(e) {
	e.length &&
		$e &&
		!Ts &&
		(console.log('Adding new records'), e.forEach((t) => console.log($s(t))));
}
function ci(e, t) {
	const n = [],
		{
...
function Ei(e, t, n, r) {
	...
	return (
		o && ((s = s.map((i) => i + '-' + o)), t && (s = s.concat(s.map($t)))), s
	);
}
function ot(e) {
...
function Ar(e) {
	...
	Je(() => {
		d === !1 &&
			...
			((S.current.isPositioned = !1), m((R) => ({ ...R, isPositioned: !1 })));
	}, [d]);
	const F = l.useRef(!1);
	Je(
		() => (
	...
	const L = l.useMemo(
			() => ({ reference: P, floating: N, setReference: b, setFloating: w }),
			[b, w],
		),
	...
	fn(t) {
		...
		return (
			d === 'bottom'
				...
				? ((v = `${-u}px`), (h = i ? m : `${g}px`))
				: d === 'left' &&
				  ((v = `${r.floating.width + u}px`), (h = i ? m : `${g}px`)),
			{ data: { x: v, y: h } }
		);
	},
});
	...
	Zr = (e) => {
		const {
			...
			[y = !1, b] = cr({
				...
				onChange: (N) => {
					N
						? (u.onOpen(), document.dispatchEvent(new CustomEvent(Bt)))
						: u.onClose(),
						s == null || s(N);
				},
			}),
			w = l.useMemo(
				() => (y ? (x.current ? 'delayed-open' : 'instant-open') : 'closed'),
				[y],
			),
			C = l.useCallback(() => {
...
function uo(e) {
	const { devMode: t, className: n, field: r, messages: o, index: s } = e,
		...
		m =
			!t && (i == null ? void 0 : i.fieldValue) && Array.isArray(i.fieldValue);
	return c.jsx(fn, {
		className: $(
			n,
...
var Wt = l.forwardRef((e, t) => {
	...
	if (l.isValidElement(n)) {
		const o = oc(n),
			s = rc(r, n.props);
		return (
			n.type !== l.Fragment && (s.ref = t ? po(t, o) : o), l.cloneElement(n, s)
		);
	}
...
var zt = l.forwardRef((e, t) => {
	...
	if (l.isValidElement(n)) {
		const o = yc(n),
			s = xc(r, n.props);
		return (
			n.type !== l.Fragment && (s.ref = t ? xo(t, o) : o), l.cloneElement(n, s)
		);
	}
	return l.Children.count(n) > 1 ? l.Children.only(null) : null;
});
...
function jl(e) {
		...
		l.useEffect(
			function () {
				if (e.inert) {
					...
					var h = ei([e.lockRef.current], (e.shards || []).map(Qn), !0).filter(
						Boolean,
					);
					return (
	...
	var a = l.useCallback(function (h, x) {
			...
			var S = Kn(N, P);
			if (!S) return !0;
			if ((S ? (E = N) : ((E = N === 'v' ? 'h' : 'v'), (S = Kn(N, P))), !S))
				return !1;
			if (
				(!r.current && 'changedTouches' in h && (w || C) && (r.current = E), !E)
			)
				return !0;
			var O = r.current || E;
	...
	Ll = l.forwardRef((e, t) => {
		...
		return c.jsx(Ko, {
			...
			onInteractOutside: (s) => {
				...
				const i = s.target;
				((d = n.triggerRef.current) == null ? void 0 : d.contains(i)) &&
					s.preventDefault(),
...
function ns(e) {
	...
	function f(m) {
		...
		if (p) {
			const v = d.findIndex(
				(h) =>
					h.timestamp && de(h.timestamp.fieldValue, 'time').includes(p.trim()),
			);
			console.log('index', v),
			...
			return (
				Object.entries(n).forEach(([m, p]) => {
					f = f.filter((g) => {
						var v;
						return (
							((v = g[m]) == null ? void 0 : v.fieldValue.toString()) === p
						);
					});
				}),
										...
										children: E.map((P, N) =>
											c.jsxs(
												Vt,
												{
													...
													children: [
														$e &&
															c.jsx(fn, { className: y, children: kt(P) || b }),
														v.map((S) =>
															c.jsx(
																uo,
